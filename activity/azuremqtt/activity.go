...
var log = logger.GetLogger("activity-azuremqtt")
...
func (a *Activity) Eval(ctx activity.Context) (done bool, err error) {
	...
	methodType := input.Type_of_Opeartion
	topic := a.settings.Topic
	if params := input.TopicParams; len(params) > 0 {
		topic = a.topic.String(params)
	}
	if token := a.client.Publish(topic, byte(), , input.Message); token.Wait() && token.Error() != nil {
